...
func (h *RFQHandler) ListRFQs(w http.ResponseWriter, r *http.Request) {
	...
	status := q.Get("status")
	minBudgetStr := q.Get("min_budget")
	maxBudgetStr := q.Get("max_budget")
	...
	if myOnly {
		...
		whereClauses = append(whereClauses, fmt.Sprintf("r.buyer_id=$%d", argIdx))
		args = append(args, companyID)
		argIdx++
	} else {
		whereClauses = append(whereClauses, "r.is_published=true")
	}

	if search != "" {
		whereClauses = append(whereClauses, fmt.Sprintf("(r.title ILIKE $%d OR r.description ILIKE $%d)", argIdx, argIdx))
	...
	if categoryID != "" {
		whereClauses = append(whereClauses, fmt.Sprintf("r.category_id=$%d", argIdx))
		args = append(args, categoryID)
		argIdx++
	}

	if minBudgetStr != "" {
		if mb, err := strconv.ParseFloat(minBudgetStr, 64); err == nil {
			whereClauses = append(whereClauses, fmt.Sprintf("(r.budget_to >= $%d OR r.budget_from >= $%d)", argIdx, argIdx))
			args = append(args, mb)
			argIdx++
		}
	}
	...
	if maxBudgetStr != "" {
		if mb, err := strconv.ParseFloat(maxBudgetStr, 64); err == nil {
			whereClauses = append(whereClauses, fmt.Sprintf("(r.budget_from <= $%d OR r.budget_to <= $%d)", argIdx, argIdx))
			args = append(args, mb)
			argIdx++
		}
	}
	...
	switch sortBy {
	case "budget_asc":
		orderSQL = "ORDER BY COALESCE(r.budget_from, r.budget_to, 0) ASC"
	case "budget_desc":
		orderSQL = "ORDER BY COALESCE(r.budget_to, r.budget_from, 0) DESC"
	case "deadline_asc":
		orderSQL = "ORDER BY r.proposals_deadline ASC NULLS LAST"
	case "newest":
		orderSQL = "ORDER BY r.created_at DESC"
	}

	query := fmt.Sprintf(`SELECT r.id, r.buyer_id, COALESCE(c.name,''), r.title, COALESCE(r.description,''),
